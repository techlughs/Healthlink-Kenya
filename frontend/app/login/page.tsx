...
export default function LoginPage() {
    ...
    return (
        <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-black px-4">
            ...
            {/* Deliberately subtle — not a real security boundary, just keeps
                the admin entry point out of casual view. Actual protection is
                server-side (see /api/admin/** role check). */}
            <Link
                href="/admin/login"
                aria-label="Admin"
                className="fixed bottom-4 right-4 z-20 flex h-6 w-6 items-center justify-center rounded-full text-white/15 transition hover:text-white/50"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
